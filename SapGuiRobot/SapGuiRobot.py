...
class SapGuiRobot:
    ...
    def __init__(
        ...
        self.subrc: int = 0
        self.explicit_wait: float = self.set_explicit_wait(explicit_wait=explicit_wait)
        self.connection_number: int = self.connection_number(connection_number=connection_number)
        self.session_number: int = self.session_number(session_number=session_number)
        ...
        self.screenshot: screenshot.Screenshot = screenshot.Screenshot()
        ...
        self.screenshot.set_screenshot_directory(screenshot_dir)
    ...
    @property
    def explicit_wait(self, explicit_wait: float = 0.0) -> None:
        try:
            self.explicit_wait = float(explicit_wait)
            logger.debug(f"explicit_wait time set to: {explicit_wait}")
        except TypeError:
            self.explicit_wait = float(0.0)
            logger.debug(f"Unable to set explicit_wait time with your given value. {explicit_wait} cannot be converted to a float.")
        except Exception as err:
            self.explicit_wait = float(0.0)
            logger.debug(f"Unable to set explicit_wait time -> {err}")
    
    @property
    def connection_number(self, connection_number: int = 0) -> None:
        try:
            self.connection_number = int(connection_number)
            logger.debug(f"connection_number set to: {connection_number}")
        except TypeError:
            self.connection_number = int(0)
            logger.debug(f"Unable to set connection_number with your given value. {connection_number} cannot be converted to a int.")
        except Exception as err:
            self.connection_number = int(0)
            logger.debug(f"Unable to set connection_number ->  {err}")
    
    @property
    def session_number(self, session_number: int = 0) -> None:
        try:
    ...
    def get_element_type(self, id: str) -> Union(str, None):
        try:
    ...
    def connect_to_existing_connection(self, connection_name: Optional[str] = None) -> None:
        ...
        if self.connection.Description == self.connection_name:
            ...
            self.take_screenshot(screenshot_name="connect_to_existing_connection_error")
    ...
    def open_connection(self, connection_name: Optional[str] = None):
        ...
        if connection_name:
            self.connection_name = connection_name
        try:
            self.connection = self.sapapp.OpenConnection(self.connection_name, True)
        except Exception as err:
            self.take_screenshot(screenshot_name="open_connection_error")
    ...
    def get_value(self, id: str) -> str | bool:
        try:
            ...
            self.take_screenshot(screenshot_name="get_value_error")
    ...
    def set_cell_value(self, table_id, row_num, col_id, text):
        if self.is_element(element=table_id):
            try:
                ...
                self.take_screenshot(screenshot_name="set_cell_value_error")
